...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out1, catche1 = affine_relu_forward(X.reshape(N, D1), W1, b1)
        scores, catche2 = affine_forward(out1, W2, b2)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        softmax_output = np.exp(shift_scores) / np.sum(np.exp(shift_scores), 1).reshape(-1, 1)
        ...
        dscores /= N
        dH, dW2, db2 = affine_backward(dscores, catche2)
        dW2 += self.reg * W2
        dtmpX, dW1, db1 = affine_relu_backward(dH, catche1)
        dW1 += self.reg * W1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, self.num_layers):
            self.params['W'+str(i)] = weight_scale * np.random.randn(cur_input_dim, hidden_dims[i-1])
            self.params['b'+str(i)] = np.zeros(hidden_dims[i-1])
            cur_input_dim = hidden_dims[i-1]
        self.params['W'+str(self.num_layers)] = weight_scale * np.random.randn(cur_input_dim, num_classes)
    ...
    def loss(self, X, y=None):
        ...
        # layer, etc.                                                              #
        ############################################################################
        outs = [X.reshape(X.shape[0], self.params['W1'].shape[0])]
        caches = [0]
        drop_caches = [0]
        ...
        for i in range(1, self.num_layers):
            ...
            if self.use_dropout:
                tmp_out, tmp_drop_cache = dropout_forward(tmp_out, self.dropout_param)
...
# others' code
    ...
    #         if self.normalization=='batchnorm':
    #             dx, dw, db, dgamma, dbeta = affine_relu_backward(dhout, ar_cache[lay])
    #         else:
