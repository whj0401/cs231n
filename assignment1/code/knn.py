...
for i in range(num_folds):
    X_train_tmp = np.vstack(X_train_folds[0:i] + X_train_folds[i+1:])
    y_train_tmp = y_train_folds[0:i] + y_train_folds[i+1:]
    y_train_tmp = np.reshape(y_train_tmp, len(y_train)-len(y_train_folds[i]))
